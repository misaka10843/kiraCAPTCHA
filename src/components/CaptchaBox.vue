<template>
  <div class="captcha-box" :style="captchaBoxStyle">
    <div class="head-box">
      <p style="font-size: 12px">选择包含</p>
      <p>
        <strong style="font-size: 28px">{{ character }}</strong>
      </p>
      <p style="font-size: 12px">的所有图片。</p>
      <img src="/assets/icon/KiraCaptcha.svg" class="cover" />
    </div>
...
<script>
export default {
  props: ['jsonFileCountProps', 'jsonBaseUrlProps', 'BoxStyle'],
  data() {
    return {
      character: null,
      images: [],
  ...
  methods: {
    ...
    checkAnswer() {
      ...
      //判断错误图片选择情况
      const incorrectSelectedCount = this.images.filter(
        (image) => image.selected && !this.correctImageIds.includes(image.id)
      ).length
      //判断正确图片选择情况
      const correctSelectedCount = this.images.filter(
        (image) => image.selected && this.correctImageIds.includes(image.id)
      ).length
      ...
      const selectedIds = this.images
        .filter((image) => image.selected)
        .map((image) => image.id)
      const isSubset = this.correctImageIds.every((id) =>
        selectedIds.includes(id)
      )
      const isSuperset =
        selectedIds.every((id) =>
          this.images.some((image) => image.id === id)
        ) && !this.correctImageIds.some((id) => !selectedIds.includes(id))
      if (!isSubset) {
        // 用户选择的图片不包含正确的图片
        warning.style.display = 'block'
